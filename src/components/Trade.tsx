const Trade = () => {
  const renderBuySellButtons = () => (
    <div className="flex gap-2 justify-start items-center">
      ...
      <button
        type="button"
        className="font-medium text-gray-4 border-b-2 border-b-transparent"
      >
        Sell
      </button>
    </div>
  ...
  const renderWallet = () => (
    <div className="flex gap-4 justify-between items-center w-[330px]">
      ...
      <div className=" flex gap-1 justify-between items-center text-[14px]">
        <button
          type="button"
          className="w-[32px] h-[32px] bg-gray-8 rounded-full"
        >
          C
        </button>
        <button
          type="button"
          className="[py-[6px] px-[12px] w-[55px] h-[32px] bg-gray-8 rounded-full"
        >
          MAX
        </button>
      </div>
  ...
  const renderCurrencySelect = () => {
    return (
      <div className="w-[330px] h-[131px] rounded-xl border border-gray-6" />
    );
  };
